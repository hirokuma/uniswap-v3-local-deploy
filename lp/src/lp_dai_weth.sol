...
contract LiquidityDaiWeth is IERC721Receiver {
    ...
    /// For this example we are providing 1000 DAI and 1000 WETH9 in liquidity
        ...
        )
    {
        ...
        {
            uint160 sqrt = TickMath.getSqrtRatioAtTick(0);

            address poolAddr = nonfungiblePositionManager.createAndInitializePoolIfNecessary(a1, a2, poolFee, sqrt);
            pool = IUniswapV3Pool(poolAddr);
console.log('@@@ mintNewPosition 0');
console.logAddress(address(pool));

            PoolAddress.PoolKey memory poolKey =
                PoolAddress.PoolKey({token0: a1, token1: a2, fee: poolFee});
            IUniswapV3Pool pool2 = IUniswapV3Pool(PoolAddress.computeAddress(nonfungiblePositionManager.factory(), poolKey));
console.log('@@@ mintNewPosition 1');
console.logAddress(address(pool2));
        ...
        uint256 amount0ToMint = 0.01 * 1e18;
        uint256 amount1ToMint = 0.01 * 1e18;

        // transfer tokens to contract
        TransferHelper.safeTransferFrom(DAI, msg.sender, address(this), amount0ToMint);
        TransferHelper.safeTransferFrom(WETH9, msg.sender, address(this), amount1ToMint);
        ...
        // Note that the pool defined by DAI/WETH9 and fee tier 0.3% must already be created and initialized in order to mint
console.log('###1');
